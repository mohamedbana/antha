...
# This script creates a complete anthadoc app in $APPDIR.
# It copies the cmd/anthadoc and src/pkg/go/... sources from GOROOT,
# synthesizes an app.yaml file, and creates the .zip, index, and
# configuration files.
#
# If an argument is provided it is assumed to be the app-engine anthadoc directory.
...
# copied to $APPDIR. A corresponding anthadoc configuration file is created
# in $APPDIR/appconfig.go.

ZIPFILE=anthadoc.zip
INDEXFILE=anthadoc.index
SPLITFILES=index.split.
GODOC=code.google.com/p/go.tools/cmd/anthadoc
...
fetchGodoc() {
	echo "*** Fetching anthadoc (if not already in GOPATH)"
	unset GOBIN
	go=$APPENGINE_SDK/go
...
makeIndexfile() {
	...
	GOPATH= anthadoc -write_index -index_files=$APPDIR/$INDEXFILE -zip=$APPDIR/$ZIPFILE
}
...
// (generated by $GOROOT/src/cmd/anthadoc/setup-anthadoc-app.bash)
