...
func _PickColoniesSteps(_ctx context.Context, _input *PickColoniesInput, _output *PickColoniesOutput) {
	...
	_output.ColoniesinRecoveryMedia = make([]*wtype.LHSolution, 0)
	var colonyinrecoverymedia *wtype.LHSolution

	for _, colonylocation := range _input.ColonyLocations {

		togetplateid := mixer.Sample(_input.Colonytype, _input.ColonyVolume)

		id := togetplateid.LContainer.Plateid

		colony := wtype.NewLHComponent()

		colony.LContainer.ID = colonylocation
...
type PickColoniesOutput struct {
	ColoniesinRecoveryMedia []*wtype.LHSolution
}
...
func init() {
	addComponent(Component{Name: "PickColonies",
