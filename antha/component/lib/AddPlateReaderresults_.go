...
func _AddPlateReaderresultsSteps(_ctx context.Context, _input *AddPlateReaderresultsInput, _output *AddPlateReaderresultsOutput) {
	...
	optimalwavelength := marsdata.FindOptimalWavelength(_input.WellForScanAnalysis[0], _input.Blanks[0], "Raw Data")
	...
	for _, run := range runs {
		...
		*/
		...
		blankcorrected, err := marsdata.BlankCorrect(samples, _input.Blanks, _input.WavelengthtoUse, _input.ReadingTypeinMarsFile)

		// path length correct
		pathlength, err := platereader.EstimatePathLength(_input.PlateType, _input.VolumePerwell)
		...
		run.AddResponseValue(_input.Responsecolumntofill, pathlengthcorrect.Reading)
		//	}
	}
	//	}
	...
	_ = doe.DXXLSXFilefromRuns(runs, _input.OutputFilename)
	...
	addComponent(Component{Name: "AddPlateReaderresults",
		...
		Desc: ComponentDesc{
			...
			Params: []ParamDesc{
				...
				{Name: "WellForScanAnalysis", Desc: "= []string{\"A1\"}\n", Kind: "Parameters"},
