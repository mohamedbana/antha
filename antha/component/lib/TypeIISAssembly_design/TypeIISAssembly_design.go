// antha/component/lib/TypeIISAssembly_design/TypeIISAssembly_design.go: Part of the Antha language
...
func (e *TypeIISAssembly_design) steps(p TypeIISAssembly_designParamBlock, r *TypeIISAssembly_designResultBlock) {
	...
	status, numberofassemblies, sitesfound, newDNASequence := enzymes.Assemblysimulator(assembly)
	...
	if r.Status != "all parts available" {
		...
		r.Status = fmt.Sprintln(
			...
			"Back up parts found (RFP which work)", r.BackupParts,
