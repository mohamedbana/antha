// antha/build/build.go: Part of the Antha language
...
func (ctxt *Context) gopath() []string {
	...
	for _, p := range ctxt.splitPathList(ctxt.GOPATH) {
		...
		if strings.HasPrefix(p, "~") {
			...
			// The antha command diagnoses this situation and prints a
			// useful error.
...
// ImportDir is like Import but processes the Go/Antha package found in
// the named directory.
...
// containing no buildable Go/Antha source files. (It may still contain
...
func (e *NoGoError) Error() string {
	return "no buildable Antha source files in " + e.Dir
}
...
// If returnImports is true and name denotes a Go/Antha program, matchFile reads
...
// (to avoid including a Go/Antha package clause doc comment),
...
func (ctxt *Context) match(name string, allTags map[string]bool) bool {
	...
	// Unlike in Antha identifiers, all digits are fine (e.g., "386").
	for _, c := range name {
