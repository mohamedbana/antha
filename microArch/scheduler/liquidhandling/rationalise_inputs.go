// /anthalib/liquidhandling/rationalise_inputs.go: Part of the Antha language
...
func Rationalise_Inputs(lhr *LHRequest, lhp *liquidhandling.LHProperties, inputs ...*wtype.LHComponent) *LHRequest {
	...
	ip := make(map[string]*wtype.LHPlate)
	...
	for i := 0; i < len(inputs); i++ {
		...
		if !((n.Kind() == reflect.Ptr && n.Elem().Name() == "wtype.LHWell") || n.Name() == "liquidhandling.wtype.LHWell") {
			logger.Fatal("Rationalise_inputs: cannot use non-microplate formats in liquid handler... yet!")
