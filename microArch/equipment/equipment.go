...
// Package equipment defines the generic representation of a piece of
// equipment.
package equipment
...
// NewActionDescription creates a new action with the given data
...
// action. This is related to how you can ask an equipment to carry out an
// action. The actionData in ActionDescription should meet the constraints that
...
// Equipment is something capable of performing different actions under
// different restrictions and explaining what its capabilities and the
// restrictions on them are
type Equipment interface {
	// GetID returns the string that identifies a piece of equipment. Ideally
	// uuids v4 should be used.
	GetID() string
	// Perform an action in the equipment. Actions might be transmitted in
	// blocks to the equipment The grouping of the actions (as a set, plate or
	// whatever) is not performed at the equipment driver level or is it?
	Do(actionDescription ActionDescription) error
	// Can queries a piece of equipment about an action execution. The
	// description of the action must meet the constraints of the piece of
	// equipment.
	Can(ac ActionDescription) bool
	// Status should give a description of the current execution status and any
	// future actions queued to the device
	Status() string
	// Init driver will be initialized when registered
	Init() error
	// Shutdown disconnect, turn off, signal whatever is necessary for a
	// graceful shutdown
	Shutdown() error
}
