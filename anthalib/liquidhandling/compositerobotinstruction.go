...
func (ins TransferInstruction)GetParameter(s string)interface{}{
	switch(s){
		case "TYPES"	: return ins.What 
		case "VOLUMES"	: return ins.Volume
		case "VOLUNTS"	: return ins.VolumeUnit
		case "POSFROM"	: return ins.PltFrom
		case "POSTO"	: return ins.PltTo
		case "WELLSFROM": return ins.WellFrom
		case "WELLSTO"	: return ins.WellTo
		case "PARAMS"	: return ins.Prms
		default: raiseError(fmt.Sprintf("Illegal parameter: %s", s))
	}
	return nil
}
